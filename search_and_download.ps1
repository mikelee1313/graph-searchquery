<#
.SYNOPSIS
This script performs a search query on OneDrive items using Microsoft Graph API , Downloads the file then exports the results to a CSV file.
...
# Function to export the search results to a CSV file and download files
function ExportResultSet($results) {
    ...
    $Results.value.hitsContainers.hits.resource | ForEach-Object {
        ...
        if ($fileUrl) {
            foreach ($fileUrl in $Results.value.hitsContainers.hits.resource.webUrl) {
                # Get the file ID from the search results to download the file
                $fileId = $results.value.hitsContainers.hits.hitId
                $filename = $fileId + "_" + $results.value.hitsContainers.hits.resource.name
                $driveid = $results.value.hitsContainers.hits.resource.parentReference.driveId
                $getfileurl = "https://graph.microsoft.com/v1.0/drives/$driveId/items/$fileId/content"
                Invoke-RestMethod -Method Get -Uri $getfileurl -Headers $headers -OutFile "$downloadPath\$filename"
                Write-Host -ForegroundColor Green "File Downloaded Successfully"
            }
        }
